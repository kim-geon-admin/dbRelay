...
**저장은 항상 성공합니다.** 미리보기 화면은 셀 값의 형식이나 타입을 검사하지 않습니다. 대상 컬럼이 받아들일 수 없는 값이라도 그대로 저장되고, 오류는 실제 대상 DB에 대해 **Run**에서 보고됩니다. 값을 지우고 저장하면 그 셀은 `null`이 됩니다.

날짜·시각 컬럼은 다음 형식으로 표시되고 같은 형식으로 편집합니다.

| 대상 컬럼 타입 | 표시·입력 형식 | 예 |
| --- | --- | --- |
| `DATE` | `YYYY-MM-DD HH:MM:SS` | `2026-09-01 14:30:25` |
| `TIMESTAMP` | `YYYY-MM-DD HH:MM:SS.ffffff` | `2026-09-01 14:30:25.123000` |

Oracle 드라이버가 시각 값을 밀리초 단위로 읽어오므로 소수점 아래 마지막 세 자리는 항상 `000`이며, 편집한 값도 같은 단위로 저장됩니다. `TIMESTAMP WITH TIME ZONE` 컬럼은 시간대까지 포함한 문자열로 표시됩니다.
...
저장한 미리보기 값을 실행할 때는 대상 Oracle 컬럼의 타입도 확인합니다. `NUMBER`, `FLOAT`, `BINARY_FLOAT`, `BINARY_DOUBLE` 대상 컬럼에 연결된 이름 있는 바인드는 안전하게 숫자로 변환되고, 안전한 JavaScript 정수 범위를 넘는 정수는 큰 정수로 전달됩니다. 날짜·시각 값은 Oracle의 `DATE`·`TIMESTAMP` 바인드로 그대로 전달되고, 그 밖의 타입은 문자열로 전달되며 빈 셀은 `null`로 유지됩니다. 대상 컬럼 정보를 완전히 확인할 수 없으면 원래의 문자열·`null` 값을 그대로 사용합니다. 숫자 컬럼에 숫자로 해석할 수 없는 값을 입력하면 대상 DML을 시작하기 전에 실행이 거부됩니다. 이 변환은 저장한 미리보기의 이름 있는 바인드에만 적용되며 SQL 리터럴에는 적용되지 않습니다.
...
- Insert에서 `SELECT * FROM 테이블` 형태를 쓰면 열 목록이 없으므로 Target SQL이 비어 있고 안내 문구가 표시됩니다. **미리보기**를 실행하고 **저장**하면 미리보기의 열로 `INSERT` 문이 생성됩니다. `WHERE`, `ORDER BY` 같은 뒤따르는 절이나 테이블 별칭이 있어도 됩니다. 조인, 콤마 조인, `UNION`, 서브쿼리를 읽는 Source SQL은 대상 테이블을 특정할 수 없으므로 안내 문구가 그대로 유지되고, 생성에 실패해도 이미 작성해 둔 Target SQL은 지워지지 않습니다.
